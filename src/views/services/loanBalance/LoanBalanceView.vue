<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      <Refresher />
      <ion-card class="card" :style="{backgroundColor: theme.primaryColor}">

          <div v-if="results.length === 0">
              <div class="no-data-text">
                  <h5 :style="{color: theme.primaryFontColor}">No data available</h5>
              </div>
          </div>

          <div v-else>
              <div v-for="(result, index) in results" :key="index" class="card-inner content-container">
                <ion-col size="6">
                  <ion-row>
                    <ion-col size="6">
                      <p>Name:</p>
...
methods: {
  ...
  async fetchdata(id) {
    try {
        ...
        const matchingEntries = allData.filter(entry => entry.id === parseInt(id, 10));
